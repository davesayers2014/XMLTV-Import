...
class EPGImport:
    ...
    def legacyDownload(self, result, afterDownload, downloadFail, sourcefile, filename, deleteFile=True):
        ...
        if sourcefile.startswith("https") and sslverify:
            ...
            downloadPage(six.ensure_binary(sourcefile), filename, sniFactory).addCallbacks(afterDownload, downloadFail, callbackArgs=(filename, True))
        else:
       ...
       # sourcefile = sourcefile.encode('utf-8')
        ...
        if ip6:
            ...
            if sourcefile.startswith("https") and sslverify:
                ...
                domain = parsed_uri.hostname
                sniFactory = SNIFactory(domain)
                downloadPage(six.ensure_binary(sourcefile6), filename, sniFactory, headers={'host': host}).addCallback(afterDownload, filename, True).addErrback(self.legacyDownload, afterDownload, downloadFail, sourcefile, filename, True)
            else:
                downloadPage(six.ensure_binary(sourcefile6), filename, headers={'host': host}).addCallback(afterDownload, filename, True).addErrback(self.legacyDownload, afterDownload, downloadFail, sourcefile, filename, True)
